...
Rules:
- PLAYBOOK ADHERENCE: a TOP-CANDIDATE PLAYBOOK + EXPERT-ANSWER PRINCIPLES are given in the user message. Match that quality bar exactly — hit the full answer depth (recommendation → drivers → evidence → risks → next steps), include the spike moves, avoid the listed common mistakes, and obey the ANTI-SLOP bans (no ungrounded "synergy/leverage/holistic", no hedging, every number sourced or labeled ESTIMATE).
