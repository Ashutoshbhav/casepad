...
> ## 🎙️ VOICE LAYER — BARGE-IN CONFIRMATION WINDOW ADDED — 2026-09-05
> **Why:** research pass (GitHub issues on livekit/agents + pipecat-ai/pipecat, plus independent Reddit voice-engineering threads) found that false-positive barge-in is a field-wide unsolved problem even for the production frameworks CasePad's own turn-taking design was benchmarked against — one cited figure puts ~89% of naively-detected interruptions as false (backchannel, noise, echo, or the model responding before the user actually finished). CasePad's `onSpeechRealStart` was firing `onBargeIn()` on a single confidence gate (MIN_SPEECH_MS + threshold), the same single-gate pattern the research flagged as insufficient specifically for barge-in, since a false positive there is uniquely costly (it stops the AI's real audio), unlike a false positive during normal listening.
> **Shipped:** `src/lib/voice/barge-in-confirm.ts` (new, pure state machine, unit-tested, same convention as `rambling-tracker.ts`) requires confident speech to hold UNBROKEN for `BARGE_IN_CONFIRM_MS` (350ms) before a barge-in actually fires, on top of the existing MIN_SPEECH_MS gate, not instead of it. Wired into `live-mic-input.tsx`: `onSpeechRealStart` during `interviewer_speaking` now only ARMS the confirmation window (`bargeInPendingRef`/`bargeInConfirmRef`) instead of calling `onBargeIn()` directly; `onAmplitude` feeds the pending window and fires `onBargeIn()` exactly once, the moment it confirms. A single non-confident frame resets the streak (deliberately strict, not averaged), and the window is reset on segment end/misfire so nothing carries over into the next segment. Only active while a barge-in is actually pending — normal listening pays zero extra bookkeeping.
> **Not done in this pass (deliberately scoped out):** the pre-VAD addressee/echo-detection layer the same research surfaced (filtering background speech/TTS echo before it ever reaches turn detection) — real browser-level echo cancellation/AGC/noise-suppression is already on by default via `@ricky0123/vad-web`'s `getStream()`, confirmed by reading the installed package, so that specific gap was smaller than expected; a dedicated addressee-detection model is a bigger lift, not bundled into this change. Also did not touch `REDEMPTION_MS`/`MIN_SPEECH_MS` themselves.
> **Verified:** `tsc --noEmit` clean, 289/289 tests pass (was 282, +7: 6 new for `barge-in-confirm.ts` + 1 new test file counted). **NOT verified:** live with a real mic — same standing gap as the rest of the voice layer; the research-backed recommendation this change is paired with (make "someone talks over it mid-sentence" a standing pre-release test, not just clean-turn testing) still needs a real spoken session to actually run.
